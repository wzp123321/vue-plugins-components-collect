<template>
  <div class="threejs-raycaster">
    ...
    <div class="hint-panel">
      ...
      <p>🖱️ <b>右键拖拽</b>: 旋转视角</p>
      <p>🖱️ <b>滚轮</b>: 缩放</p>
      <p>🖱️ <b>按住 Shift + 左键拖拽</b>: 移动选中物体</p>
      <p>🖱️ <b>W / E / R 键</b>: 切换 平移/旋转/缩放 模式</p>
      <p v-if="selectedName" style="color: #ffcc80">✅ 当前选中: <b>{{ selectedName }}</b></p>
    </div>
  </div>
</template>
...
function clearHover() {
  if (hoveredObject) {
    hoveredObject.mesh.material['emissive']?.set(hoveredObject.originalEmissive)
    hoveredObject = null
  }
}
...
function setHover(intersection: Intersection) {
  ...
  hoveredObject = info
  // 自发光设为淡蓝色，表示鼠标悬停
  info.mesh.material['emissive']?.set(0x224466)
  log(`Hover: "${info.name}"`)
}
  ...
  if (event.button === 0 && event.shiftKey) {
    ...
    if (intersection) {
      ...
      if (info && transformControls) {
        ...
        log(`开始拖拽: "${info.name}", offset=(${dragOffset.x.toFixed(2)}, ${dragOffset.y.toFixed(2)}, ${dragOffset.z.toFixed(2)})`)
      }
    }
  }
}
...
function createScene() {
  ...
  // ---- 可交互物体 ----
  const objects: Array<{ geometry: any; material: MeshStandardMaterial; position: Vector3; name: string; color: number }> = [
    {
