...
const exportExcel = async () => {
  ...
  const worksheet = workbook.addWorksheet('区域费用排名成本明细');
  ...
  const secondLevelHeaders = [
    ...
    { name: '分项成本', colSpan: 4, rowSpan: 1 }, // 合并4列
  ];

  // 第三级表头行（第三行）
  const thirdLevelHeaders = ['电用量', '电成本', '水用量', '水成本'];
  ...
  // 设置数据行
  exportData.value.forEach((row, rowIndex) => {
    const dataRow = rowIndex + 4; // 从第4行开始（因为表头占用了1-3行）

    // 设置每列数据
    worksheet.getCell(dataRow, 1).value = rowIndex + 1;
    worksheet.getCell(dataRow, 2).value = row.object;
    worksheet.getCell(dataRow, 3).value = row.totalCost;
    worksheet.getCell(dataRow, 4).value = row.electricityUsage;
    worksheet.getCell(dataRow, 5).value = row.electricityCost;
    worksheet.getCell(dataRow, 6).value = row.waterUsage;
    worksheet.getCell(dataRow, 7).value = row.waterCost;

    // 设置数字格式（如果需要）
    worksheet.getCell(dataRow, 3).numFmt = '#,##0';
    worksheet.getCell(dataRow, 5).numFmt = '#,##0';
    worksheet.getCell(dataRow, 7).numFmt = '#,##0';
  });
  ...
  // 设置边框
  const startRow = 1;
  const endRow = 3 + exportData.value.length;
  const startCol = 1;
  const endCol = 7;
