...
// 生成一组随机数
export const getArrayRound = (): number[] => {
  let arr = [];
  for (let i = 0; i < 12; i++) {
...
export const FGetAuthorization = () => {
  const loginName = FGetCookie('ems-loginName');
  const authorization = `${new Date().getTime()}_${getRandomSixNum()}_${loginName}`;
