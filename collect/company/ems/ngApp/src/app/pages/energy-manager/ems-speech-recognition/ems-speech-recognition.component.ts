...
export class EmsSpeechRecognitionComponent implements OnInit, OnDestroy {
  ...
  filterStr(str: string) {
    ...
    if (isAndroid) {
      ...
      for (let i = 0; i < str.length; i++) {
        specialStr += str.substr(i, 1).replace(regRule, '');
      }
    }
    if (isiOS) {
      const regRule =
  ...
  textareaChange(item: any) {
    ...
    let BuildName = this.filterStr(item);
    // let BuildName = item.detail.value;
    BuildName = BuildName;
    let curr: any = BuildName;
    ...
    //console.log(this.emsTextarea)
    // this.emsTextarea.nativeElement.value = curr
  }
  textareaValueInput(item: any) {
    // console.log(item)
  }
  textareaBlur(item: any) {
    // this.loadingShow = false;
    //console.log(this.textareaShowText);
    const inputValue = this.textareaShowText.replace(/\'|/g, '');
    const h = this.elementRef.nativeElement.querySelector('#textareaFocus');
    if (h) {
      h.value = inputValue;
    }
    this.textareaShowText = inputValue;
    this.textareaValue = inputValue;

    this.detailChange.emit(this.textareaValue);
    //  this.el.nativeElement.value = value.replace(/\'/g,"");
    // this.checkedBack.emit(value.replace(/\'/g,""));
    setTimeout(() => {
      this.isFous = false;
    }, 600);
  }
