import { FGetSessionStorageData } from '../index';
...
export default class GatewayUtil {
  ...
  static decryptSecret() {
    ...
    const accessTokenSecretKey = FGetSessionStorageData('energy-ems-access-secret') as string;
    console.log(
      Base64Util.encode(key),
      accessTokenSecretKey,
      AesGcmNoPaddingUtil.decrypt(accessTokenSecretKey, Base64Util.encode(key)),
      '-----',
    );
    return AesGcmNoPaddingUtil.decrypt(accessTokenSecretKey, Base64Util.encode(key));
  }
  ...
  static buildShareText() {
    ...
    let clientPublicKey = DHUtil.getClientPublicKey(DHUtil.getClientPrivateKey(), DHUtil.getP(), DHUtil.getG());
  ...
  static decryptShareRes(encrypted: string) {
    ...
    let shareKey = DHUtil.getShareKey(bigInt(serverPublicKey), DHUtil.getClientPrivateKey(), DHUtil.getP());
  ...
  static buildAuthorizeText(appId: string, decryptedShareResponse: ASSESS_DecryptShareRes) {
    ...
    let plainText =
      ...
      '&request_id=' +
      this.uuid32();
    let dhEncrypt = DHUtil.encrypt(decryptedShareResponse.shareId, decryptedShareResponse.shareKey);
    let aesEncrypt = AesGcmNoPaddingUtil.encrypt(plainText, decryptedShareResponse.randomAESBase64Key);
  ...
  static buildClientAccessToken() {
    ...
    if (item) {
      try {
        ...
        let serverToken = this.decryptAuthorizeRes(item, decryptShareRes.randomAESBase64Key);
        let accessToken = this.uuid32() + '.' + new Date().getTime() + '.' + serverToken;
        let encrypt = AesGcmNoPaddingUtil.encrypt(accessToken, decryptShareRes.randomAESBase64Key);
        let encryptRequestIdText = DHUtil.encrypt(decryptShareRes.shareId, decryptShareRes.shareKey);
