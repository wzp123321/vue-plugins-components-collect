...
const cfu = {
  ...
  formatter: {
    yAxisFormatNumber: function (val, index, opts) {
      var data =
        opts && opts.series && opts.series.length > 0 && opts.series[0] && opts.series[0].data
          ? [...opts.series[0].data]
          : [];
      ...
      if (data && data.length > 0) {
        ...
        if (mergeArrUnit === 'k') {
          texts = Number(val / 1000) + 'k';
        } else if (mergeArrUnit === 'M') {
          texts = Number(val / 1000000) + 'M';
        } else if (mergeArrUnit === 'G') {
          texts = Number(val / 1000000000) + 'G';
        } else if (mergeArrUnit === 'T') {
          texts = Number(val / 1000000000000) + 'T';
        } else if (mergeArrUnit === 'P') {
          texts = Number(val / 1000000000000000) + 'P';
        } else if (mergeArrUnit === 'E') {
          texts = Number(val / 1000000000000000000) + 'E';
        } else {
          texts = Number(val);
        }
      }
    ...
    yAxisFormatPercent: function (val, index, opts) {
    ...
    // 自定义格式化百分比tooltip
    tooltipFormatPercent: function (item, category, index, opts) {
      return item.name + '：' + (item.data === null ? '-' : item.data + '%');
    },
    // 自定义格式化千分位数据tooltip
    tooltipFormatThousand: function (item, category, index, opts) {
      var formatUtil = (value) => {
        if (value || value === 0) {
      ...
      return item.name + '：' + (item.data === null ? '-' : formatUtil(item.data));
    },
    // ring自定义格式化百分比tooltip
    ringTooltipFormatPercent: function (item, category, index, opts) {
      return (
