<template>
  <div id="ems-pagination">
    ...
    <div class="tool-group">
      ...
      >
        <i v-if="!isJumpLeftActive" class="iconfont icon-shenglvehao"></i>
        <i v-if="isJumpLeftActive" class="iconfont icon-doubleleft"></i>
      </button>
      ...
      >
        <i v-if="!isJumpRightActive" class="iconfont icon-shenglvehao"></i>
        <i v-if="isJumpRightActive" class="iconfont icon-a-doubleright"></i>
      </button>
...
const jumpToPage = (event: Event) => {
  ...
  if (checkJumpVerification(element.value)) {
    ...
    index.value = +element.value <= 0 ? 1 : +pageCount();
    element.value = index.value.toString();
    emit('update:currentPage', index.value);
    emit('current-change', index.value);
  }
...
watch(
  ...
  (newVal) => {
    ...
    if (pageCount() === 0 && props.currentPage !== 1) {
