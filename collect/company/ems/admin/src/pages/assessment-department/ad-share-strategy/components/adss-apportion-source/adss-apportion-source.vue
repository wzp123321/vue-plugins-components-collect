<template>
  <div class="adss-apportion-source">
    <div class="aas-radio-select">
      <span class="aas-radio-prop">分摊源类型</span>
    ...
    <!-- 变量公式 -->
    <div class="aas-variable-content" v-if="shareSourceInfo.apportionedSourceType === sourceType.变量">
      <div class="aas-variable-content-left">
        <div class="aas-title-name">赋值变量</div>
        <div class="aas-use-explain">使用说明：赋予实际意义的变量“V0”、“V1”用于右侧公式编辑</div>
        <ul v-show="shareSourceInfo.apportionedSourceVarList?.length !== 0">
          <li v-for="(item, index) in shareSourceInfo.apportionedSourceVarList">
            <div class="aas-tag-img"></div>
            <div class="aas-var-name">V{{ index }}</div>
            <div
              class="aas-point-info"
            ...
            <div class="aas-delete" @click="deleteVar(index)"></div>
          </li>
        </ul>
        <div class="aas-no-data-container" v-show="shareSourceInfo.apportionedSourceVarList?.length === 0">
          <no-data :width="100" :height="100"></no-data>
        </div>
      </div>
      <div class="aas-variable-content-right">
        <div class="aas-title-name">公式编辑框</div>
        <div class="aas-use-explain">使用说明：在下面输入框中输入公式，且只能使用下列计算符号、和左侧变量名</div>
        <div class="aas-compute-symbol">
          <div class="aas-symbol-item">+</div>
          <div class="aas-symbol-item">-</div>
          <div class="aas-symbol-item">*</div>
          <div class="aas-symbol-item">/</div>
          <div class="aas-symbol-item">(</div>
        ...
        <div class="aas-input-container" :class="{ wrongful: !isLegal }">
          <el-input
            spellcheck="false"
...
import AdssPointSelect from '../adss-point-select/adss-point-select.vue';
import { SelectedPoint } from '../adss-point-select/adss-point-select-api';
import { cloneDeep } from 'lodash';
import { PropType, reactive, ref } from 'vue';
import { SourceItem, sourceType } from '../adss-add/adss-add.api';
