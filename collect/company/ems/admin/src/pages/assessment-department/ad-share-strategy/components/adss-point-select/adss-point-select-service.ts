...
import { pointService, QueryParams, SelectedPoint, Adssa_PointItem } from './adss-point-select-api';
...
export class PointService {
  ...
  async getDeviceList() {
    ...
    try {
      const res = await pointService.getDeviceList({
        standardPointCode: '',
        concentratorId: this.deviceParams.id,
      });
      if (res && res.code === 200) {
  ...
  async getPointList() {
    this.pointLoading.value = true;
    try {
      const res = await pointService.getPointList({
        energyCode: this.pointParams.energyCode,
        id: this.pointParams.id ? Number(this.pointParams.id) : null,
      });
      if (res && res.code === 200) {
