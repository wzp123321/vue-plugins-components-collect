...
export default defineComponent({
  ...
  setup(props) {
    ...
    const currentLeft = computed(() => {
      ...
      let precent = 0;
      if (medianValue >= maxValue && measureValue > maxValue) {
        precent = 97;
      } else if (medianValue <= minValue && minValue !== maxValue) {
        precent = 0;
      } else if (minValue < medianValue < maxValue && measureValue < maxValue) {
        precent = 100 * (Number(medianValue) / Number(maxValue));
      } else if (minValue < medianValue < maxValue && measureValue > maxValue) {
        precent = 97 * (Number(medianValue) / Number(maxValue));
      } else {
        precent = 100;
      }
      return precent;
    ...
    const progressWidth = computed(() => {
      ...
      if (!props.isLegend) {
        ...
        if (
          ...
          Object.prototype.toString.call(medianValue) === '[object Null]'
        ) {
          width = 0;
        } else if (
          ...
          measureValue <= Number(medianValue) &&
          Number(maxValue) !== Number(minValue)
        ) {
          width = 0;
        } else {
    ...
    const status = computed(() => {
      const { maxValue, medianValue, measureValue } = explainInfo.value;
      let s = BENCH_STATUS.NORMAL;
      if (!props.isLegend) {
        if (measureValue >= maxValue) {
          s = BENCH_STATUS.HEIGHEST;
        } else if (measureValue <= medianValue) {
