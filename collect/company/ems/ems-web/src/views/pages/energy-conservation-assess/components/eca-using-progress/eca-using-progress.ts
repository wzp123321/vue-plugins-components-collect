...
export default defineComponent({
  ...
  setup(props) {
    const { title, dateScope, currentDate, quotaTitle, unit, isHistoryDate } = props;
    // 字体颜色数组
    const colors = [
    ...
    const status = computed(() => {
      let s = 0;
      const cosumeValue = compCosume.value && compCosume.value.itemValue ? compCosume.value.itemValue : 0;
      const idealValue = compIdeal.value && compIdeal.value.itemValue ? compIdeal.value.itemValue : 0;
      const quotaValue = compQuota.value && compQuota.value.itemValue ? compQuota.value.itemValue : 0;
      if (idealValue !== 0 && cosumeValue > idealValue && idealValue < quotaValue) {
        s = 1;
      }
