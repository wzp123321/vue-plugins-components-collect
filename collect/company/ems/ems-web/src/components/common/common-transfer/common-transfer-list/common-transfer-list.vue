 ...
 * @LastEditors: yut
 * @LastEditTime: 2024-01-15 09:48:04
 * @Descripttion: 
-->
<template>
...
const handleCheckAllChange = (val: boolean) => {
  if (
    ...
    props.maxLength &&
    dataSource.value.length > props.maxLength
  ) {
...
const handleChecked = (checked: boolean, item: Common_IListDataType) => {
  if (
    Object.prototype.toString.call(props.maxLength) === '[object Number]' &&
    props.maxLength &&
    checkedValues.value.length > props.maxLength
  ) {
    const index = checkedValues.value.findIndex((items) => {
