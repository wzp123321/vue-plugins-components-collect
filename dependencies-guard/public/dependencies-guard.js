...
function parseOriginalArgs() {
  ...
  try {
    ...
    if (Array.isArray(parsed.original)) return parsed.original.map(item => String(item))
    if (Array.isArray(parsed.cooked)) return parsed.cooked.map(item => String(item))
    return []
  } catch {
...
function parseInstallContext(originalArgs) {
  ...
  const installIndex = lowerArgs.findIndex(arg => arg === 'install' || arg === 'i')
  ...
  const fromLifecycle = lifecycleEvent === 'preinstall' || lifecycleEvent === 'dependencies'
  const fromCommand = npmCommand === 'install' || npmCommand === 'i'
  const isInstall = installIndex !== -1 || fromLifecycle || fromCommand
  const isNoSave = lowerArgs.includes('--no-save') || envFlag('npm_config_no_save')
  const isSaveDev = lowerArgs.includes('--save-dev') || lowerArgs.includes('-d') || envFlag('npm_config_save_dev')
  const isGlobal = lowerArgs.includes('--global') || lowerArgs.includes('-g') || envFlag('npm_config_global')
  const isSaveOptional = lowerArgs.includes('--save-optional') || lowerArgs.includes('-o') || envFlag('npm_config_save_optional')
  const shouldSkip = !isInstall || isNoSave || isSaveDev || isGlobal || isSaveOptional
  const specs = []
  if (installIndex !== -1) {
...
function main() {
  ...
  if (illegalInstall.length > 0) {
    const removed = removeFromDependencies(packageJsonPath, pkgJson, illegalInstall)
    if (removed.length > 0) {
  ...
  } else if (installContext.lifecycleEvent === 'dependencies' && illegalExisting.length > 0) {
    // dependencies 生命周期下兜底清理现存非法项，保证回滚结果一致
    const removed = removeFromDependencies(packageJsonPath, pkgJson, illegalExisting)
    if (removed.length > 0) {
      console.error('已从 package.json 的 dependencies 移除非法依赖:')
      removed.forEach(name => console.error(`  - ${name}`))
    }
