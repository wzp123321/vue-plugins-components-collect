      ...
      res.on('end', () => {
        try {
          ...
          if (!Array.isArray(config.dependencies)) {
            reject(new Error('whitelist.json 格式错误，dependencies 必须是字符串数组'))
            return
          }
          // 仅允许字符串项，避免对象配置导致规则理解不一致
          const invalid = config.dependencies.filter(item => typeof item !== 'string' || item.trim() === '')
          if (invalid.length > 0) {
            reject(new Error('whitelist.json 格式错误，dependencies 只能包含非空字符串'))
            return
          }
          ...
          resolve({
            ...
            list: Array.from(new Set(config.dependencies.map(item => item.trim())))
          })
        } catch (error) {
