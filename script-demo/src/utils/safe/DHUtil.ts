...
export default class DHUtil {
  ...
  static getClientPrivateKey() {
    ...
    return bigInt(GatewayUtil.decryptSecret());
  }
   ...
   * @return base64转码的密文
   */
  static encrypt(plainText: string, shareKey: any) {
    const aesKey = this.getDhSecretKey(shareKey);
   ...
   * @return utf8编码的明文
   */
  static decrypt(base64EncryptedText: string, shareKey: any): string {
    const aesKey = this.getDhSecretKey(shareKey);
    const key = CryptoJS.enc.Utf8.parse(aesKey);
    const decrypted = CryptoJS.AES.decrypt(base64EncryptedText, key, {
      mode: CryptoJS.mode.ECB,
      padding: CryptoJS.pad.Pkcs7,
    });
  ...
  static getDhSecretKey(shareKey: any) {
    if (shareKey instanceof String) {
      return CryptoJS.MD5(shareKey as any).toString();
    }
