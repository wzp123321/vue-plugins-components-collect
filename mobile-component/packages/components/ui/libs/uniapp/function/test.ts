 ...
 */
export function date(value: number | string | null | undefined) {
 ...
 * 验证身份证号码
 */
export function idCard(value: string | number) {
  return /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test(String(value));
}
 ...
 * 验证一个值范围[min, max]
 */
export function range(value: number, param: [number, number]) {
 ...
 * 验证一个长度范围[min, max]
 */
export function rangeLength(value: string, param: [number, number]) {
...
export function empty(value: unknown) {
  switch (typeof value) {
    ...
    case 'boolean':
      ...
      break;
    case 'number':
      if (value === 0 || isNaN(value as number)) return true;
      break;
    case 'object':
 ...
 * 是否短信验证码
 */
export function code(value: string | number, len = 6) {
 ...
 * @param {Object} value
 */
export function func(value: unknown) {
 ...
 * @param {Object} value
 */
export function promise(value: unknown) {
  const maybePromise = value as { then?: unknown; catch?: unknown };
  return object(value) && func(maybePromise.then) && func(maybePromise.catch);
}
 ...
 * @return {Boolean}
 */
export function regExp(o: unknown) {
