...
<template>
  <view class="tsm-switch" :class="bemClass" :style="switchStyle" @tap="clickHandler">
    ...
    <view class="tsm-switch-text" v-if="props.inactiveText">
...
const switchStyle = computed(() => {
  ...
  const size = addUnit(String(width));
  return addStyle({
    width: size,
...
const nodeStyle = computed(() => {
  const { width } = getSwitchSize(props.size);
  const nodeSize = getNodeWidthBySize(props.size);
  const translateX = isChecked.value ? width - nodeSize - 2 * innerPadding : 0;
  return {
