...
export function createSeats(): [Seat, Seat, Seat, Seat] {
  ...
  const make = (i: 0 | 1 | 2 | 3): Seat => ({
    ...
    fireCooldown: 0,
    lastInputAt: 0,
    account: { balance: 0, deposited: 0, spent: 0 },
  });
...
export class World {
  ...
  /**
   * 尝试发射：先检查余额是否够，够则扣费并发射。
   * 返回 { ok, cost, bullet }：ok=false 时 cost/bullet 都为 0/null。
   */
  tryFire(
    seat: Seat,
    damage: number,
    ...
    if (seat.fireCooldown > 0) return { ok: false, cost: 0, bullet: null };
    if (!seat.online) return { ok: false, cost: 0, bullet: null };
    const cost = costOfShot(seat.multiplier);
    if (seat.account.balance < cost) {
      return { ok: false, cost, bullet: null };
    }
    seat.account.balance -= cost;
    seat.account.spent += cost;
    const bl = this.fire(seat, damage, speed);
    return { ok: !!bl, cost, bullet: bl };
  }

  /** 给玩家账户充值（返回 true 表示成功） */
  topUp(seat: Seat, amount: number, bonus: number): void {
    const total = amount + bonus;
    seat.account.balance += total;
    seat.account.deposited += total;
  }

  /** 重置玩家账户（用于 leave/cleanup） */
  resetAccount(seat: Seat): void {
    seat.account.balance = 0;
    seat.account.deposited = 0;
    seat.account.spent = 0;
  }

  /** 给玩家赠送初始币（首次进入房间时调用） */
  grantInitial(seat: Seat): void {
    seat.account.balance = COIN.initialBalance;
    seat.account.deposited = COIN.initialBalance;
    seat.account.spent = 0;
  }
}
