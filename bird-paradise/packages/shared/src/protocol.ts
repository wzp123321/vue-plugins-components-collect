// 共享协议类型 - 客户端与服务端一致
// 由 packages/shared 提供，客户端与服务端均通过 @bird-paradise/shared 引入
import type { PlayerAccount } from "./config.js";
...
export interface ClientToServerEvents {
  join: (
    data: { roomId: string; name: string; preferredSeat?: SeatIndex },
    cb: (resp: JoinResponse) => void,
  ) => void;
  leave: () => void;
  ...
  /** 玩家充值 */
  topUp: (data: { amount: number }, cb: (resp: TopUpResponse) => void) => void;
}
...
export interface ServerToClientEvents {
  ...
  /** 玩家账户变化（充值 / 扣费 / 入场初始） */
  accountUpdated: (data: { seat: SeatIndex; account: PlayerAccount }) => void;
  /** 系统消息（如房间满/已开始） */
