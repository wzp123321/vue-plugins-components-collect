    ...
    mounted(el, binding) {
      ...
      domDrag.onmousedown = (e: MouseEvent) => {
        if (
          ...
          (e.target as HTMLElement).className.indexOf('el-dialog__header') !==
            -1 &&
          (e.target as HTMLElement).className !== 'el-dialog__headerbtn'
        ) {
          ...
          document.onmousemove = (moveE: MouseEvent) => {
            ...
            l =
              l < 0
                ? 0
                : l > screenWidth - Number(domWidth)
                ? screenWidth - Number(domWidth)
                : l
            ...
            t =
              t > screenHeight - Number(domHeight)
                ? screenHeight - Number(domHeight)
                : t
            t = t < 0 ? 0 : t
...
const getInitialMarginLeft = (el: HTMLElement) => {
  ...
  if (value === 'auto') {
    ...
    const width = removeSuffix(
      clientWidth,
      domWidth,
      domWidth.indexOf('%') !== -1 ? '%' : 'px'
    )
