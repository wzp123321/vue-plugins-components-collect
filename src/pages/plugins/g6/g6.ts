...
// utils
import nodeStateStyles from './utils';
// service
import { G6DataSourceVO, CommonObject } from './g6.api';
...
export default defineComponent({
  ...
  props: {
    // 数据源
    dataSource: {
      type: Object as PropType<G6DataSourceVO>,
      default: {},
    },
    // 方向
    direction: {
  ...
  setup(props) {
    ...
    const calculateG6TreeData = () => {
      ...
      // 重写展开、收缩图标，增加 +、- 与圆的空隙
      G6.Marker.collapse = (x: number, y: number, r: number) => {
        return [
          ['M', x - r, y],
          ['a', r, r, 0, 1, 0, r * 2, 0],
          ['a', r, r, 0, 1, 0, -r * 2, 0],
      ...
      G6.Marker.expand = (x: number, y: number, r: number) => {
        return [
          ['M', x - r, y],
          ['a', r, r, 0, 1, 0, r * 2, 0],
          ['a', r, r, 0, 1, 0, -r * 2, 0],
      ...
      G6.registerNode(
        'flow-rect',
        {
          ...
          draw(cfg: any, group: any) {
            ...
            // 标记是否满足筛选条件
            let hasIcons = false;
            if (differenceRatio > (Number(props.totalRatio) || 5)) {
              icons.push({
                name: '差',
                fill: 'rgba(250, 173, 20)',
                nodeName: 'icon-difference',
                tooltip: `节点能耗总值相对下级节点能耗总值之和的差值大于${Number(props.totalRatio) || 5}%`,
              });
              hasIcons = true;
            }
            if (dailyGrowth > (Number(props.dailyRatio) || 30)) {
              icons.push({
                ...
                tooltip: `节点能耗相对昨日增长超过${Number(props.dailyRatio) || '30'}%`,
              });
              hasIcons = true;
            }
            if (dailyGrowth < -(Number(props.dailyRatio) || 30)) {
            ...
            const drawBody = (
              distance: number,
              { order, title, num, markShow = true, mark, markValue, hasIcon, rectName, rectWidth }: CommonObject
            ) => {
              // 总（kWh）
              ...
              if (markShow) {
                ...
                const formattedMarkValue = markValue;
    ...
    const initG6Chart = () => {
      ...
      try {
        ...
        graph.on('card:click', async (e: IG6GraphEvent) => {
          ...
          // 支路树不能跳转
          if (cardData && !cardData?.limitFlag) {
            const { treeId } = cardData;
          }
        });
        loading.value = false;
    ...
    const onFilter = () => {
      ...
      if (props.dataSource) {
        ...
        if (filteredLength.value === 0) {
          message.warning('未筛选到符合条件的节点');
        } else {
          message.success('筛选完成，请根据节点角标查看结果');
        }
      }
    };
    /**
