...
const placeholderTagMatcher = new MatchDecorator({
  regexp: /\[\[(.+?)\]\]/g,
  decoration: (match) => Decoration.replace({ widget: new PlaceholderTag(match[1]) }),
});
...
const placeholderFnMatcher = new MatchDecorator({
  regexp: /\{\{(.+?)\}\}/g,
  decoration: (match) => Decoration.replace({ widget: new PlaceholderFn(match[1]) }),
});
...
function myCompletions(context: CompletionContext) {
  ...
  const before = context.matchBefore(/[s](?:u(?:m)?)?|[i](?:f)?/gi);
...
export const useCodemirror = () => {
  ...
  const insertText = (text: string, type: "fn" | "tag" = "tag") => {
    if (view.value) {
      const content = type === "tag" ? `[[${text}]]` : `{{${text}}}()`;
