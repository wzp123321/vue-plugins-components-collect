...
const placeholderTag = ViewPlugin.fromClass(
  ...
  class {
    ...
    update(update: ViewUpdate) {
      ...
      this.placeholders = placeholderTagMatcher.updateDeco(update, this.placeholders);
    }
  },
  // 配置对象，用于定义插件的行为
  {
...
const placeholderFn = ViewPlugin.fromClass(
  ...
  class {
    ...
    update(update: ViewUpdate) {
      // 调用placeholderMatcher.updateDeco方法，根据传入的update和当前的placeholders更新装饰集，并重新赋值给placeholders属性
      this.placeholders = placeholderFnMatcher.updateDeco(update, this.placeholders);
    }
  },
  // 配置对象，用于定义插件的行为
  {
  ...
  const insertText = (text: string, type: 'fn' | 'tag' = 'tag') => {
    ...
    if (view.value) {
      ...
      if (!selection.main.empty) {
        ...
        const anchor = type === 'tag' ? from + content.length : from + content.length - 1;
        ...
        const anchor = type === 'tag' ? pos + content.length : pos + content.length - 1;
        const transaction = view.value.state.update({
